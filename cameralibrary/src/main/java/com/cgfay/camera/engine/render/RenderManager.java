...
public final class RenderManager {
    ...
    private void initFilters(Context context) {
        ...
        //LUT 颜色查表滤镜
        mFilterArrays.put(RenderIndex.LookupFilterIndex, new GLImage64LookupTableFilter(context));
    }
     ...
     * @param color
     */
    public synchronized void changeLookupFilter(Bitmap bitmap) {
        if (mFilterArrays.get(RenderIndex.LookupFilterIndex) != null) {
            mFilterArrays.get(RenderIndex.LookupFilterIndex).release();
            mFilterArrays.put(RenderIndex.LookupFilterIndex, null);
        }
        if (bitmap == null) {
            return;
        }
        ...
        filter.initFrameBuffer(mTextureWidth, mTextureHeight);
        filter.onDisplaySizeChanged(mViewWidth, mViewHeight);
        mFilterArrays.put(RenderIndex.LookupFilterIndex, filter);
    }
    ...
    public int drawFrame(int inputTexture, float[] mMatrix) {
        ...
        if (!mCameraParam.showCompare) {
            ...
            //LUT 颜色查找表滤镜
            if (mFilterArrays.get(RenderIndex.LookupFilterIndex) != null) {
                currentTexture = mFilterArrays.get(RenderIndex.LookupFilterIndex).drawFrameBuffer(currentTexture, mVertexBuffer, mTextureBuffer);
            }
        }
